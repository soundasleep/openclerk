...
/**
 * Individual BTCT securities job.
 */

$exchange = "individual_btct";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_btct WHERE user_id=? AND id=?");
...
$q->execute(array('securities_btct', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_btct ID=" . htmlspecialchars($account['security_id']));
} else {
