...
/**
 * Summary job: convert all cryptocurrencies to USD (via BTC) using Vircurex, and add any USD balances.
 */
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		throw new JobException("There is no recent ticker balance for usd/btc on vircurex - cannot convert");
	}
...
crypto_log("Total converted USD Vircurex balance for user " . $job['user_id'] . ": " . $total);
