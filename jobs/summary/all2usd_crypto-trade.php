...
/**
 * Summary job: convert all cryptocurrencies to USD (via BTC) using Crypto-Trade, and add any USD balances.
 */
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		throw new JobException("There is no recent ticker balance for usd/btc on crypto-trade - cannot convert");
	}
...
crypto_log("Total converted USD Crypto-Trade balance for user " . $job['user_id'] . ": " . $total);
