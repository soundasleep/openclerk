...
/**
 * Summary job: convert all cryptocurrencies to EUR (via BTC) using Mt.Gox, and add any EUR balances.
 */
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		throw new JobException("There is no recent ticker balance for eur/btc on mtgox - cannot convert");
	}
...
crypto_log("Total converted EUR Mt.Gox balance for user " . $job['user_id'] . ": " . $total);
