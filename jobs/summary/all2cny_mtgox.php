...
/**
 * Summary job: convert all cryptocurrencies to CNY (via BTC) using Mt.Gox, and add any CAD balances.
 */
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		throw new JobException("There is no recent ticker balance for cny/btc on mtgox - cannot convert");
	}
...
crypto_log("Total converted CNY Mt.Gox balance for user " . $job['user_id'] . ": " . $total);
