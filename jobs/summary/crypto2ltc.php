...
// FTC is first converted to BTC then converted to LTC at BTC-e ticker rate sell
$q = db()->prepare("SELECT * FROM summary_instances WHERE summary_type=? AND user_id=? AND is_recent=1");
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		if ($ticker = $q->fetch()) {
			crypto_log("+ from FTC (LTC): " . ($temp / $ticker['buy']));
			$total += $temp / $ticker['buy'];
		}
	}
...
// PPC is first converted to BTC then converted to LTC at BTC-e ticker rate sell
$q = db()->prepare("SELECT * FROM summary_instances WHERE summary_type=? AND user_id=? AND is_recent=1");
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		if ($ticker = $q->fetch()) {
			crypto_log("+ from PPC (LTC): " . ($temp / $ticker['buy']));
			$total += $temp / $ticker['buy'];
		}
	}
...
// NVC is first converted to BTC then converted to LTC at BTC-e ticker rate sell
$q = db()->prepare("SELECT * FROM summary_instances WHERE summary_type=? AND user_id=? AND is_recent=1");
...
if ($balance = $q->fetch()) {
	...
	if ($ticker = $q->fetch()) {
		...
		if ($ticker = $q->fetch()) {
			crypto_log("+ from NVC (LTC): " . ($temp / $ticker['buy']));
			$total += $temp / $ticker['buy'];
		}
	}
