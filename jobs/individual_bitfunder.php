...
/**
 * Individual BitFunder securities job.
 */

$exchange = "individual_bitfunder";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_bitfunder WHERE user_id=? AND id=?");
...
$q->execute(array('securities_bitfunder', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_bitfunder ID=" . htmlspecialchars($account['security_id']));
} else {
