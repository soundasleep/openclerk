...
/**
 * Individual BTCInve securities job.
 */

$exchange = "individual_btcinve";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_btcinve WHERE user_id=? AND id=?");
...
$q->execute(array('securities_btcinve', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_btcinve ID=" . htmlspecialchars($account['security_id']));
} else {
