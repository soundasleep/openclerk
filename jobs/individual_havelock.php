...
/**
 * Individual Havelock Investments securities job.
 */

$exchange = "individual_havelock";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_havelock WHERE user_id=? AND id=?");
...
$q->execute(array('securities_havelock', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_havelock ID=" . htmlspecialchars($account['security_id']));
} else {
