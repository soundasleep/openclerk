...
/**
 * Individual Litecoininvest securities job.
 */

$exchange = "individual_litecoininvest";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_litecoininvest WHERE user_id=? AND id=?");
...
$q->execute(array('securities_litecoininvest', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_litecoininvest ID=" . htmlspecialchars($account['security_id']));
} else {
