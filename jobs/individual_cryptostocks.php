...
/**
 * Individual Cryptostocks securities job.
 */

$exchange = "individual_cryptostocks";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_cryptostocks WHERE user_id=? AND id=?");
...
$q->execute(array('securities_cryptostocks', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_cryptostocks ID=" . htmlspecialchars($account['security_id']));
} else {
