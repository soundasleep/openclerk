...
/**
 * Individual Litecoin Global securities job.
 */

$exchange = "individual_litecoinglobal";

// get the relevant address
$q = db()->prepare("SELECT * FROM accounts_individual_litecoinglobal WHERE user_id=? AND id=?");
...
$q->execute(array('securities_litecoinglobal', $account['security_id']));
$ticker = $q->fetch();

if (!$ticker) {
	throw new ExternalAPIException("Could not find any recent ticker balance for securities_litecoinglobal ID=" . htmlspecialchars($account['security_id']));
} else {
