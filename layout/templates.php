...
<form action="<?php echo htmlspecialchars(url_for('set_locale')); ?>" method="post" id="locale_selector">
	...
	<?php foreach (get_all_locales() as $locale) {
		...
		echo "<option value=\"" . htmlspecialchars($locale) . "\" class=\"locale locale-" . htmlspecialchars($locale) . "\"" . ($selected ? " selected" : "") . ">" . htmlspecialchars(t(get_locale_label($locale))) . "</option>\n";
	}
	?>
	</select>
