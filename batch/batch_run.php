...
class OpenclerkJobRunner extends JobRunner {
  ...
  function createJob($job, Connection $db, Logger $logger) {
    $logger->info(link_to(url_for('batch/batch_run', array('key' => require_get("key", false), 'job_id' => $job['id'], 'force' => 1)), "Run again"));
    $logger->info(link_to(url_for('batch/batch_run', array('key' => require_get("key", false))), "Next job"));
