 ...
 * for example storing the supported hashrate currencies - so that they do not have to be
 * refetched every time.
 */
class PersistentAccountTypeHashrates {
  ...
  function storeSupportedHashrateCurrencies($currencies, Logger $logger) {
    $logger->info("Storing " . count($currencies) . " hashrate currencies persistently");

    // find all existing currencies
    $existing = $this->getSupportedHashrateCurrencies(true);
    ...
    foreach ($existing as $currency) {
      if (array_search($currency, $currencies) === false) {
        ...
        $q = $this->db->prepare("DELETE FROM account_hash_currencies WHERE exchange=? AND currency=?");
    ...
    foreach ($currencies as $currency) {
      if (array_search($currency, $existing) === false) {
        ...
        $q = $this->db->prepare("INSERT INTO account_hash_currencies SET exchange=?, currency=?");
        $q->execute(array($this->exchange->getCode(), $currency));
      }
    }
   ...
   */
  function getSupportedHashrateCurrencies($force = false) {
    if ($this->cached_currencies === null || $force) {
      $this->cached_currencies = array();
      $q = $this->db->prepare("SELECT * FROM account_hash_currencies WHERE exchange=?");
