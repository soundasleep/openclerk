...
/**
 * Updates individual securities data for `accounts_individual_havelock`
 * to use their security name rather than ID.
 */
class MigrateHavelockIndividual extends \Db\Migration {
  ...
  function apply(\Db\Connection $db) {
    $q = $db->prepare("ALTER TABLE accounts_individual_havelock ADD security VARCHAR(64) NOT NULL");
    if (!$q->execute()) {
      throw new \Exception("Could not add security row to table");
    }

    // this is easier than writing sql
    $q = $db->prepare("SELECT * FROM securities_havelock");
    if (!$q->execute()) {
    ...
    foreach ($securities as $security) {
      $q = $db->prepare("UPDATE accounts_individual_havelock SET security=:security WHERE security_id=:id");
    ...
    $q = $db->prepare("ALTER TABLE accounts_individual_havelock DROP security_id");
    if (!$q->execute()) {
